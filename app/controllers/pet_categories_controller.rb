class PetCategoriesController < ApplicationController
  ...
  def show
      respond_to do |format|
        format.json {render json: @pet_category}
        format.html 
    end
  end
  ...
  # DELETE /pet_categories/1
  # DELETE /pet_categories/1.json
  def destroy
    @pet_category.destroy
    respond_to do |format|
      format.html { redirect_to pet_categories_url, notice: 'Pet category was successfully destroyed.' }
  ...
  private
    ...
    def pet_category_params
      params.require(:pet_category).permit(:category)
    end
end
